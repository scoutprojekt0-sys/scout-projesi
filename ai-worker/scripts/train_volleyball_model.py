...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train volleyball player/ball YOLO model")
    parser.add_argument(
        "--data",
        default=str(Path(__file__).resolve().parents[1] / "datasets" / "volleyball_detection.yaml"),
...
def main() -> None:
    ...
    resolved_dataset_yaml = _resolved_dataset_yaml("volleyball", dataset_path, args.ball_boost)
    overrides = {
        "model": args.model,
    ...
    class NextScoutDetectionTrainer(DetectionTrainer):
        def _do_train(self):
            ...
            LOGGER.info(f"\n{self.epochs - self.start_epoch} epochs completed in {(time.time() - self.train_time_start) / 3600:.3f} hours.")
