...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare football dataset frames from source videos")
    parser.add_argument("--source-dir", required=True, help="Directory containing raw football videos")
...
def main() -> None:
    ...
    dataset_root = Path(args.output_dir).resolve() if args.output_dir else Path(__file__).resolve().parents[1] / "datasets" / "football"
    ...
    video_files = sorted(
        [path for path in source_dir.rglob("*") if path.suffix.lower() in {".mp4", ".mov", ".avi", ".mkv", ".webm"}]
    )
    if not video_files:
