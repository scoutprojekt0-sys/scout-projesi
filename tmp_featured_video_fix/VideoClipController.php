...
class VideoClipController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'file'              => ['required', 'file', 'max:102400', 'mimetypes:video/mp4,video/quicktime,video/x-msvideo,video/webm'],
            'platform'          => ['nullable', 'in:custom'],
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        $videoPath = $this->extractPublicDiskPath($clip->video_url);
        if ($videoPath !== null) {
            Storage::disk('public')->delete($videoPath);
        }

        $thumbPath = $this->extractPublicDiskPath($clip->thumbnail_url);
        if ($thumbPath !== null) {
        ...
        if (Schema::hasTable('player_profiles') && Schema::hasColumn('player_profiles', 'featured_video_clip_id')) {
            DB::table('player_profiles')
                ->where('featured_video_clip_id', $clip->id)
                ->update([
                    'featured_video_clip_id' => null,
                    'updated_at' => now(),
                ]);
        }
