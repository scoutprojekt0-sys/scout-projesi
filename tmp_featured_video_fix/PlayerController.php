...
class PlayerController extends Controller
{
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        if (is_object($player) && property_exists($player, 'photo_url')) {
            $player->photo_url = $this->publicFileUrl($player->photo_url);
        }
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'featured_video_clip_id' => [
                'sometimes',
                'nullable',
                Rule::exists('video_clips', 'id')->where(
                    fn ($query) => $query->where('user_id', $id)
                ),
            ],
        ]);
        ...
        DB::table('player_profiles')->updateOrInsert(
            ...
            [
                ...
                'featured_video_clip_id' => array_key_exists('featured_video_clip_id', $validated)
                    ? $validated['featured_video_clip_id']
                    : ($existingProfile->featured_video_clip_id ?? null),
                'updated_at' => now(),
            ]
        );
        ...
        if ($updated && property_exists($updated, 'photo_url')) {
            $updated->photo_url = $this->publicFileUrl($updated->photo_url);
        }
    ...
    public function publicProfile(int $id): JsonResponse
    {
        $player = DB::table('users')
            ...
            ->where('users.id', $id)
            ->where('users.role', 'player')
            ->select($this->publicProfileSelects())
            ->first();
        ...
        if (Schema::hasTable('player_statistics')) {
            try {
                $statsRows = DB::table('player_statistics')
                    ...
                    ->orderByDesc('season')
                    ->orderByDesc('id')
                    ->get($this->publicProfileStatsSelects());
            } catch (Throwable) {
                $statsRows = collect();
            }
        }
        ...
        $showcase = $this->buildShowcaseStatus((int) $id, $player);
        $publicPhotoUrl = $this->publicFileUrl($player->photo_url ?? null);
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'profile' => [
                    ...
                    'featured_video_clip_id' => $player->featured_video_clip_id !== null
                        ? (int) $player->featured_video_clip_id
                        : null,
                ],
                'card' => [
                    'id' => (int) $player->id,
                    ...
                    'nationality' => (string) ($player->country ?? ''),
                    'profile_photo_url' => (string) ($publicPhotoUrl ?? ''),
                    ...
                    'featured_video_clip_id' => $player->featured_video_clip_id !== null
                        ? (int) $player->featured_video_clip_id
                        : null,
                    'featured_video' => $this->buildFeaturedVideoPayload(
                        (int) $id,
                        $player->featured_video_clip_id !== null ? (int) $player->featured_video_clip_id : null
                    ),
                    'showcase' => $showcase,
    ...
    private function publicProfileSelects(): array
    {
        return [
            'users.id',
            'users.name',
            $this->selectOrNull('users', 'users.sport', 'sport'),
            $this->selectOrNull('users', 'users.gender', 'gender'),
            $this->selectOrNull('users', 'users.is_verified', 'is_verified'),
            $this->selectOrNull('users', 'users.verification_status', 'verification_status'),
            $this->selectOrNull('users', 'users.verified_at', 'verified_at'),
            $this->selectOrNull('users', 'users.contract_status', 'contract_status'),
            $this->selectOrNull('users', 'users.seeking_club', 'seeking_club'),
            $this->selectOrNull('users', 'users.city', 'city'),
            $this->selectOrNull('users', 'users.country', 'country'),
            $this->selectOrNull('users', 'users.age', 'age'),
            $this->selectOrNull('users', 'users.position', 'position', 'user_position'),
            $this->selectOrNull('users', 'users.photo_url', 'photo_url'),
            $this->selectOrNull('users', 'users.rating', 'rating', 'user_rating'),
            $this->selectOrNull('users', 'users.confidence_score', 'confidence_score'),
            $this->selectOrNull('users', 'users.updated_at', 'updated_at'),
            $this->selectOrNull('player_profiles', 'pp.birth_year', 'birth_year'),
            $this->selectOrNull('player_profiles', 'pp.position', 'position'),
            $this->selectOrNull('player_profiles', 'pp.dominant_foot', 'dominant_foot'),
            $this->selectOrNull('player_profiles', 'pp.height_cm', 'height_cm'),
            $this->selectOrNull('player_profiles', 'pp.weight_kg', 'weight_kg'),
            $this->selectOrNull('player_profiles', 'pp.current_team', 'current_team'),
            $this->selectOrNull('player_profiles', 'pp.featured_video_clip_id', 'featured_video_clip_id'),
            $this->selectOrNull('player_profiles', 'pp.bio', 'bio'),
        ];
    }

    private function buildFeaturedVideoPayload(int $playerId, ?int $videoClipId): ?array
    {
        if ($videoClipId === null) {
            return null;
        }
        ...
        return [
            ...
            'video_url' => (string) ($this->publicFileUrl($clip->video_url) ?? ''),
            'thumbnail_url' => $this->publicFileUrl($clip->thumbnail_url),
            'platform' => (string) $clip->platform,
            'duration_seconds' => $clip->duration_seconds !== null ? (int) $clip->duration_seconds : null,
            'match_date' => $clip->match_date?->toDateString(),
            'created_at' => $clip->created_at?->toIso8601String(),
        ];
    }

    private function publicProfileStatsSelects(): array
    {
        return [
            'season',
            $this->selectOrNull('player_statistics', 'league', 'league'),
            $this->selectOrNull('player_statistics', 'matches_played', 'matches_played'),
            $this->selectOrNull('player_statistics', 'matches_started', 'matches_started'),
            $this->selectOrNull('player_statistics', 'matches_benched', 'matches_benched'),
            $this->selectOrNull('player_statistics', 'goals', 'goals'),
            $this->selectOrNull('player_statistics', 'assists', 'assists'),
            $this->selectOrNull('player_statistics', 'minutes_played', 'minutes_played'),
            $this->selectOrNull('player_statistics', 'avg_rating', 'avg_rating'),
        ];
    }

    private function selectOrNull(string $table, string $selectExpression, string $column, ?string $alias = null): mixed
    {
        if (Schema::hasColumn($table, $column)) {
            return $alias === null ? $selectExpression : $selectExpression.' as '.$alias;
        }

        return DB::raw('null as '.($alias ?? $column));
    }
    ...
    public function shareAssets(Request $request): JsonResponse
    {
        ...
        $profile = DB::table('users')
            ...
            ->select([
                ...
                'users.email',
                $this->selectOrNull('users', 'users.city', 'city'),
                $this->selectOrNull('users', 'users.rating', 'rating'),
                $this->selectOrNull('users', 'users.position', 'position', 'user_position'),
                'pp.position',
                'pp.height_cm',
                'pp.current_team',
        ...
        if (Schema::hasTable('contracts')) {
            try {
                $contract = DB::table('contracts')
                    ...
                    ->select([
                        'contracts.status',
                        DB::raw('COALESCE(contracts.club_name, clubs.name) as club_name'),
                    ])
                    ->first();
            } catch (Throwable) {
                $contract = null;
